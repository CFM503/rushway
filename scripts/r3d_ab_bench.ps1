# Interleaved A/B: HEAD baseline vs Round-2 candidate 3
# (encode-buffer pool: acquire/recycle on writer path). Same session, alternating samples.
param(
    [int]$Samples = 8,
...
$BinA = Join-Path $RepoRoot "bench\oldbin\rushway_r2_base.exe"  # HEAD pre-pool
$BinB = Join-Path $RepoRoot "target\release\rushway.exe"         # encode-pool candidate
$Key = "rushway-proxy-bench-test-key"
$WorkDir = Join-Path $env:TEMP "w2bench"
New-Item -ItemType Directory -Force -Path $WorkDir | Out-Null
if (-not $OutCsv) { $OutCsv = Join-Path $RepoRoot "bench\r3d_ab_rushway.csv" }
...
foreach ($mode in $Modes) {
    $m = @{}
    foreach ($impl in @("rushway_base", "rushway_r3d")) {
    ...
    if (-not ($m.ContainsKey("rushway_base") -and $m.ContainsKey("rushway_r3d"))) { continue }
    $b = $m["rushway_base"]; $w = $m["rushway_r3d"]
    $bMed = $b.Medians; $wMed = $w.Medians
    $line = "REF mode=$mode (independent medians, informational) base(n=$($b.Rows.Count)): c1=$($bMed.c1) c8=$($bMed.c8) c32=$($bMed.c32) cpu=$($bMed.cpu) rss=$($bMed.rss) | r3d(n=$($w.Rows.Count)): c1=$($wMed.c1) c8=$($wMed.c8) c32=$($wMed.c32) cpu=$($wMed.cpu) rss=$($wMed.rss)"
    Write-Host $line
    $rawLines += $line
    ...
    foreach ($md in $metricDefs) {
        ...
        $mline = "PAIRED mode=$mode metric=$($md.Name) n=$($pairIds.Count) eff_n=$effN crit=$crit bad=$bad good=$good ties=$ties med_delta=$medDelta median_favours_r3d=$favoredDelta => $st"
        Write-Host $mline
        $rawLines += $mline
    }
...
$rawLines | Add-Content -Path (Join-Path (Split-Path $OutCsv) "r3d_ab_raw.log") -Encoding utf8

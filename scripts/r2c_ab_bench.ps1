# Interleaved A/B: HEAD (pre-coalesce) baseline vs Round-2 candidate 2
# (writer_loop cold-wake coalesce). Same session, alternating samples.
param(
    [int]$Samples = 8,
...
$BinA = Join-Path $RepoRoot "bench\oldbin\rushway_r2_base.exe"  # HEAD pre-coalesce
$BinB = Join-Path $RepoRoot "target\release\rushway.exe"         # coalesce candidate
$Key = "rushway-proxy-bench-test-key"
$WorkDir = Join-Path $env:TEMP "w2bench"
New-Item -ItemType Directory -Force -Path $WorkDir | Out-Null
if (-not $OutCsv) { $OutCsv = Join-Path $RepoRoot "bench\r2c_ab_rushway.csv" }
...
foreach ($mode in $Modes) {
    $m = @{}
    foreach ($impl in @("rushway_base", "rushway_r2c")) {
    ...
    if (-not ($m.ContainsKey("rushway_base") -and $m.ContainsKey("rushway_r2c"))) { continue }
    $b = $m["rushway_base"]; $w = $m["rushway_r2c"]
    $bMed = $b.Medians; $wMed = $w.Medians
    $line = "REF mode=$mode (independent medians, informational) base(n=$($b.Rows.Count)): c1=$($bMed.c1) c8=$($bMed.c8) c32=$($bMed.c32) cpu=$($bMed.cpu) rss=$($bMed.rss) | r2c(n=$($w.Rows.Count)): c1=$($wMed.c1) c8=$($wMed.c8) c32=$($wMed.c32) cpu=$($wMed.cpu) rss=$($wMed.rss)"
    Write-Host $line
    $rawLines += $line
    ...
    foreach ($md in $metricDefs) {
        ...
        $mline = "PAIRED mode=$mode metric=$($md.Name) n=$($pairIds.Count) eff_n=$effN crit=$crit bad=$bad good=$good ties=$ties med_delta=$medDelta median_favours_r2c=$favoredDelta => $st"
        Write-Host $mline
        $rawLines += $mline
    }
...
$rawLines | Add-Content -Path (Join-Path (Split-Path $OutCsv) "r2c_ab_raw.log") -Encoding utf8

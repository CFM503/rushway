# Interleaved A/B: goway v1.8.11 baseline vs current W3 working tree.
# Same session, alternating samples, order flipped every sample, to cancel
# machine-drift and order bias. Verdict = paired per-sample deltas with an
# exact sign test (ties dropped): a metric is REGRESSED only when the bad
# count reaches the two-sided p<=0.05 threshold for the effective n —
# i.e. regression "beyond noise" only (forward-only rule,
# way/goway/AI_HANDOFF.md). Independent medians are printed informational.
param(
    [int]$Samples = 6,
...
$BinA = Join-Path $RepoRoot "bench\oldbin\goway_v1811_ab.exe"   # baseline v1.8.11
$BinB = "D:\SOFT\AI\github\way\goway\goway.exe"                 # W3 working tree
$Key = "rushway-proxy-bench-test-key"
$WorkDir = Join-Path $env:TEMP "w2bench"
New-Item -ItemType Directory -Force -Path $WorkDir | Out-Null
if (-not $OutCsv) { $OutCsv = Join-Path $RepoRoot "bench\w3_ab_goway.csv" }
...
function Start-GowayPair([string]$Bin, [int]$Sp, [int]$Cp, [string]$Tag) {
...
function Invoke-OneRun([string]$Impl, [string]$Bin, [string]$Mode, [int]$Sample) {
    ...
    $procs = @()
    try {
        $procs = Start-GowayPair $Bin $Sp $Cp "${Impl}_$Sample"
...
foreach ($mode in $Modes) {
    for ($sample = 1; $sample -le $Samples; $sample++) {
        # Interleave with flipped order every sample to cancel drift + order bias.
        if ($sample % 2 -eq 1) {
            $order = @(
                @{ Impl = "goway_base"; Bin = $BinA },
                @{ Impl = "goway_w3";   Bin = $BinB }
            )
        } else {
            $order = @(
                @{ Impl = "goway_w3";   Bin = $BinB },
...
function Get-SignCrit([int]$n) {
    # Smallest k with 2*P(X >= k) <= 0.05, X ~ Bin(n, 0.5).
    # Returns n+1 when no such k exists (never significant at this n).
    if ($n -le 0) { return 1 }
    $logF = @(0.0)
...
# metric name, csv col, "higher better" flag, decimals
$metricDefs = @(
...
foreach ($mode in $Modes) {
    $m = @{}
    foreach ($impl in @("goway_base", "goway_w3")) {
    ...
    if (-not ($m.ContainsKey("goway_base") -and $m.ContainsKey("goway_w3"))) { continue }
    $b = $m["goway_base"]; $w = $m["goway_w3"]
    $bMed = $b.Medians; $wMed = $w.Medians
    $line = "REF mode=$mode (independent medians, informational) base(n=$($b.Rows.Count)): c1=$($bMed.c1) c8=$($bMed.c8) c32=$($bMed.c32) cpu=$($bMed.cpu) rss=$($bMed.rss) | w3(n=$($w.Rows.Count)): c1=$($wMed.c1) c8=$($wMed.c8) c32=$($wMed.c32) cpu=$($wMed.cpu) rss=$($wMed.rss)"
    Write-Host $line
    $rawLines += $line

    # NOTE: never name this $samples — collides with param([int]$Samples)
    # (PowerShell variables are case-insensitive) and array assignment
    # would then fail the [int] converter.
    ...
    foreach ($md in $metricDefs) {
        ...
        $mline = "PAIRED mode=$mode metric=$($md.Name) n=$($pairIds.Count) eff_n=$effN crit=$crit bad=$bad good=$good ties=$ties med_delta=$medDelta median_favours_w3=$favoredDelta => $st"
        Write-Host $mline
        $rawLines += $mline
    }
...
$rawLines | Add-Content -Path (Join-Path (Split-Path $OutCsv) "w3_ab_raw.log") -Encoding utf8

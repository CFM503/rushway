# Interleaved A/B: rushway v0.0.26 (HEAD, worktree-built) baseline vs current
# Round-1 working tree. Same session, alternating samples, order flipped every
# sample, to cancel machine-drift and order bias. Verdict = paired per-sample
# deltas with an exact sign test (ties dropped): a metric is REGRESSED only
# when the bad count reaches the two-sided p<=0.05 threshold for the effective
# n — i.e. regression "beyond noise" only (forward-only rule). Independent
# medians are printed informational.
param(
    [int]$Samples = 8,
...
$BinA = Join-Path $RepoRoot "bench\oldbin\rushway_v0026_ab.exe"  # baseline v0.0.26
$BinB = Join-Path $RepoRoot "target\release\rushway.exe"          # R1 working tree
$Key = "rushway-proxy-bench-test-key"
$WorkDir = Join-Path $env:TEMP "w2bench"
New-Item -ItemType Directory -Force -Path $WorkDir | Out-Null
if (-not $OutCsv) { $OutCsv = Join-Path $RepoRoot "bench\r1_ab_rushway.csv" }
...
foreach ($mode in $Modes) {
    $m = @{}
    foreach ($impl in @("rushway_base", "rushway_r1")) {
    ...
    if (-not ($m.ContainsKey("rushway_base") -and $m.ContainsKey("rushway_r1"))) { continue }
    $b = $m["rushway_base"]; $w = $m["rushway_r1"]
    $bMed = $b.Medians; $wMed = $w.Medians
    $line = "REF mode=$mode (independent medians, informational) base(n=$($b.Rows.Count)): c1=$($bMed.c1) c8=$($bMed.c8) c32=$($bMed.c32) cpu=$($bMed.cpu) rss=$($bMed.rss) | r1(n=$($w.Rows.Count)): c1=$($wMed.c1) c8=$($wMed.c8) c32=$($wMed.c32) cpu=$($wMed.cpu) rss=$($wMed.rss)"
    Write-Host $line
    $rawLines += $line
    ...
    foreach ($md in $metricDefs) {
        ...
        $mline = "PAIRED mode=$mode metric=$($md.Name) n=$($pairIds.Count) eff_n=$effN crit=$crit bad=$bad good=$good ties=$ties med_delta=$medDelta median_favours_r1=$favoredDelta => $st"
        Write-Host $mline
        $rawLines += $mline
    }
...
$rawLines | Add-Content -Path (Join-Path (Split-Path $OutCsv) "r1_ab_raw.log") -Encoding utf8

...
# Expression statements / socket ownership.
...
sub(
    "src/runtime.rs",
    r"(?m)^([ \t]*)return ip\.is_loopback\(\) \|\| ip\.is_private\(\) \|\| ip\.is_link_local\(\) \|\| ip\.is_unspecified\(\);$",
...
sub(
    "src/runtime.rs",
    r"(?m)^([ \t]*)let addr = resolve_socket\(&target\.host, target\.port\)\.await\?;$",
    r"\1let addr = resolve_socket(&target.host, target.port).await.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;",
)

sub(
    "src/quic.rs",
    r"(?ms)^\s*cfg\.initial_stream_receive_window\(VarInt::from_u64\(2 \* 1024 \* 1024\)\);\s*$\n\s*cfg\.max_stream_receive_window\(VarInt::from_u64\(8 \* 1024 \* 1024\)\);\s*$\n\s*cfg\.initial_connection_receive_window\(VarInt::from_u64\(4 \* 1024 \* 1024\)\);\s*$\n\s*cfg\.max_connection_receive_window\(VarInt::from_u64\(16 \* 1024 \* 1024\)\);",
...
sub(
    "src/main.rs",
    r"(?m)^    let args = Args::parse_from\(&raw_args\);\n    let filter =",

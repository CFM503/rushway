...
- **Process rule (user-mandated, 2026-09-23): never emit no-op placeholder tool calls.** This session repeatedly fired `wsl -d Debian -- bash -lc "sleep 0"` batches alongside real commands as filler/pre-warm — they do nothing, polluted the user's terminal view, and (being parallel `wsl.exe` launches) actively provoked the `E_UNEXPECTED`/`ChildProcess.kill` failures above. Every tool call must carry real work; when a dependent step is not ready, wait for the next turn instead of padding with stubs. Parallel-batch only genuinely independent calls.

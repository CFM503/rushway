...
pub async fn run_client(cfg: RuntimeConfig) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move { if let Err(e) = handle_local_proxy(stream, cfg2, id).await { tracing::debug!(%peer, error=%e, "proxy connection closed"); } });
    }
}
...
pub async fn run_server(cfg: RuntimeConfig) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            let result = async { let request = read_http_headers(&mut stream).await?; handle_mux_server(stream, cfg2, request).await }.await;

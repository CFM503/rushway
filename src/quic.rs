...
async fn read_quic_line(stream: &mut quinn::RecvStream, limit: usize) -> Result<String> { let mut buf=Vec::with_capacity(128); while buf.len()<limit{let mut one=[0u8;1];let n=stream.read(&mut one).await?;if n==0{bail!("QUIC stream closed while reading header")}buf.push(one[0]);if one[0]==b'\n'{return Ok(String::from_utf8(buf)?.trim().to_string())}}bail!("QUIC header too large") }

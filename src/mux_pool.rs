...
async fn send_mux_parts_reuse(writer: &Arc<Mutex<WriteHalf<TcpStream>>>, stream_id: u32, command: MuxCommand, payload: &[u8], scratch: &mut Vec<u8>) -> Result<()> {
    scratch.clear();
...
#[derive(Debug)]
struct SessionState { writer: Arc<Mutex<WriteHalf<TcpStream>>>, streams: Arc<Mutex<HashMap<u32,mpsc::Sender<OwnedMuxFrame>>>>, next_id: AtomicU32, active: AtomicUsize, closed: AtomicBool }
impl SessionState {
    async fn connect(cfg: &RuntimeConfig) -> Result<Arc<Self>> {
        ...
        let (host,path)=parse_upstream(upstream)?; let actual_host=cfg.fakehost.clone().unwrap_or_else(||host.clone());
        ...
        if opcode!=2 { bail!("invalid MUX handshake response opcode"); } transform_payload(&cipher,&mut ok); if ok!=b"OK\n" { bail!("upstream rejected MUX handshake"); }
        let state=Arc::new(Self{writer:writer.clone(),streams:Arc::new(Mutex::new(HashMap::new())),next_id:AtomicU32::new(1),active:AtomicUsize::new(0),closed:AtomicBool::new(false)});
    ...
    async fn open_stream(self:&Arc<Self>,target:&TargetAddr)->Result<(u32,mpsc::Receiver<OwnedMuxFrame>)> {
        if !self.available(){bail!("MUX session is full or closed");} let id=self.next_id.fetch_add(1,Ordering::Relaxed).max(1); let(tx,rx)=mpsc::channel(32); self.streams.lock().await.insert(id,tx); self.active.fetch_add(1,Ordering::AcqRel);
...
async fn handle_udp_proxy(mut control:TcpStream,cfg:RuntimeConfig,bind_hint:TargetAddr)->Result<()> {
    ...
    let upload=tokio::spawn(async move { let mut buf=vec![0u8;64*1024]; loop { let(n,peer)=udp_send.recv_from(&mut buf).await?; *latest_send.lock().await=Some(peer); let mut data=buf[..n].to_vec(); transform_payload(&cipher_send,&mut data); let mut w=writer_send.lock().await; write_frame(&mut *w,&data,2,true).await?; } Result::<()>::Ok(()) });
    loop { let Some((opcode,mut packet))=read_frame(&mut rd,Option::<&mut WriteHalf<TcpStream>>::None,&mut frame_buf).await?else{break;};if opcode!=2{continue;}transform_payload(&cipher,&mut packet);let(_src,payload)=parse_socks5_udp_datagram(&packet).map_err(|e|anyhow!(e.to_string()))?;if let Some(peer)=*latest_client.lock().await{let _=udp.send_to(payload,peer).await;}}
    upload.abort();control.shutdown().await.ok();Ok(())
}

async fn handle_tcp_proxy(mut local:TcpStream,cfg:RuntimeConfig,pool:Arc<MuxSessionPool>)->Result<()> {let(command,target,is_socks5)=read_proxy_request(&mut local).await?;if command==SocksCommand::UdpAssociate{return handle_udp_proxy(local,cfg,target).await;}if is_socks5{local.write_all(&socks5_success_response()).await?;}else{local.write_all(b"HTTP/1.1 200 Connection Established\r\n\r\n").await?;}let(session,id,mut rx)=pool.acquire(&target).await?;let(mut local_rd,mut local_wr)=tokio::io::split(local);let writer=session.writer.clone();let upload=tokio::spawn(async move {let mut buf=vec![0u8;cfg.buffer_size.clamp(16*1024,1024*1024)];let mut frame_scratch=Vec::with_capacity(buf.len().min(u16::MAX as usize)+7);loop{let n=local_rd.read(&mut buf).await?;if n==0{let _=send_mux_parts_reuse(&writer,id,MuxCommand::Fin,&[],&mut frame_scratch).await;break;}let mut off=0;while off<n{let end=(off+u16::MAX as usize).min(n);send_mux_parts_reuse(&writer,id,MuxCommand::Data,&buf[off..end],&mut frame_scratch).await?;off=end;}}Result::<()>::Ok(())});let mut result=Result::<()>::Ok(());while let Some(frame)=rx.recv().await{match frame.command{MuxCommand::Data=>{if let Err(e)=local_wr.write_all(frame.payload()).await{result=Err(e.into());break;}},MuxCommand::Fin=>{let _=local_wr.shutdown().await;break;},MuxCommand::Rst=>break,MuxCommand::Syn=>{}}}upload.abort();session.close_stream(id).await;result}

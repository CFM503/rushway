...
impl SessionState {
    ...
    fn available(&self) -> bool {
        !self.closed.load(Ordering::Acquire) && self.active.load(Ordering::Acquire) < MAX_STREAMS_PER_SESSION
    }
...
impl MuxSessionPool {
    ...
    async fn acquire(self: &Arc<Self>, target: &TargetAddr) -> Result<(Arc<SessionState>, u32, mpsc::Receiver<OwnedMuxFrame>)> {
        ...
        loop {
            ...
            let result = new_session.open_stream(target).await?;
            self.sessions.lock().await.push(new_session.clone());
...
async fn handle_tcp_proxy(mut local: TcpStream, cfg: RuntimeConfig, pool: Arc<MuxSessionPool>) -> Result<()> {
    ...
    while let Some(frame) = rx.recv().await {
        match frame.command {
            MuxCommand::Data => { if let Err(e) = local_wr.write_all(frame.payload()).await { result = Err(e.into()); break; } }
            MuxCommand::Fin => { let _ = local_wr.shutdown().await; break; }
...
pub async fn run_client(cfg: RuntimeConfig) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_tcp_proxy(stream, cfg2, pool2).await { tracing::debug!(%peer, error=%e, "pooled proxy connection closed"); }
        });
    }
}
...
where
    ...
    S: Into<String>,
{
    args.into_iter().map(Into::into).map(|arg| {
...
async fn load_json(path: PathBuf) -> Result<RuntimeConfig> {
    ...
    if let Some(x) = v.get("tcp_keepalive").or_else(|| v.get("tcpKeepAlive")).and_then(|x| x.as_bool()) { cfg.tcp_keepalive = x }
    if let Some(x) = v.get("socket_buffer").or_else(|| v.get("socketBuffer")).and_then(|x| x.as_u64()) { cfg.socket_buffer = usize::try_from(x).map_err(|_| anyhow!("socket_buffer out of range"))? }
    if let Some(x) = v.get("dns").or_else(|| v.get("dnsServer")).and_then(|x| x.as_str()) { dns::configure(Some(x.into())).await? }
...
async fn main() -> Result<()> {
    ...
    if let Some(v) = args.buffer_kib { cfg.buffer_size = v.saturating_mul(1024).max(16 * 1024); }
    if let Some(v) = args.socket_buffer_kib { cfg.socket_buffer = v; }
    cfg.tcp_nodelay = !args.no_tcp_nodelay;
    cfg.tcp_keepalive = !args.no_tcp_keepalive;
    if args.no_mux { cfg.mux = false; } else if args.mux { cfg.mux = true; }
    if args.allow_open { cfg.allow_open = true; }
    if let Some(v) = args.max_conn { cfg.max_connections = v; }
    if args.no_block_local { cfg.block_local = false; } else if args.block_local { cfg.block_local = true; }
    if let Some(v) = args.connection_timeout { cfg.connection_timeout = v; }
    if let Some(v) = args.dns { dns::configure(Some(v)).await?; }
    ...
    if let Some(upstream) = cfg.upstream.as_deref() {
        ...
        if cfg.mux { return mux_pool::run_client(cfg).await; }
...
mod tests {
    ...
    fn parses_policy_args() {
        let args = Args::parse_from(["rushway", "--no-tcp-nodelay", "--no-tcp-keepalive", "--max-conn", "32", "--no-block-local"]);

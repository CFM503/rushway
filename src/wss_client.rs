...
async fn handle_connection(mut local: TcpStream, cfg: WssConfig, id: u32) -> Result<()> {
    ...
    let syn = MuxFrame::new(id, MuxCommand::Syn, syn.encode().map_err(|e| anyhow!(e.to_string()))?).map_err(|e| anyhow!(e.to_string()))?;
    ...
    let upload = tokio::spawn(async move {
        ...
        loop {
            ...
            if n == 0 { let _ = send_mux(&writer_up, &MuxFrame::new(id, MuxCommand::Fin, Vec::new()).unwrap()).await; break; }
            let mut off = 0;
            while off < n { let end = (off + u16::MAX as usize).min(n); let frame = MuxFrame::new(id, MuxCommand::Data, buf[off..end].to_vec()).unwrap(); send_mux(&writer_up, &frame).await?; off = end; }
        }
        Result::<()>::Ok(())
    });
    loop {
        ...
        match frame.command { MuxCommand::Data => local_wr.write_all(&frame.payload).await?, MuxCommand::Fin => { local_wr.shutdown().await?; break; }, MuxCommand::Rst => break, MuxCommand::Syn => {} }
    }
    upload.abort();
    Ok(())
}
...
pub async fn run_client_from_config(cfg: RuntimeConfig, verify_ssl: bool) -> Result<()> {
    ...
    tracing::info!("RushWay WSS client proxy listening on {}:{}", wc.proxy_host, wc.proxy_port);
    let counter = Arc::new(std::sync::atomic::AtomicU32::new(1));
    loop {
        let (stream, peer) = listener.accept().await?;
        let id = counter.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        let cfg2 = wc.clone();
        tokio::spawn(async move { if let Err(e) = handle_connection(stream, cfg2, id).await { tracing::debug!(%peer, error=%e, "WSS proxy connection closed"); } });
    }
}

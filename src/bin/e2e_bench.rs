...
fn child_path() -> io::Result<std::path::PathBuf> {
    let path = std::env::current_exe()?.parent().map(|p| p.join(if cfg!(windows) { "rushway.exe" } else { "rushway" }))
...
fn stress_mode() -> bool {
    matches!(std::env::var("RUSHWAY_E2E_STRESS").ok().as_deref(), Some("1" | "true" | "yes"))
}

fn stress_pattern() -> &'static str {
    match std::env::var("RUSHWAY_E2E_STRESS_PATTERN").ok().as_deref() { Some("staged") => "staged", _ => "burst" }
}

fn upstream_url(port: u16) -> String {
    match transport_mode() { "quic" => format!("quic://127.0.0.1:{port}"), "wss" => format!("wss://127.0.0.1:{port}/"), _ => format!("ws://127.0.0.1:{port}/") }
}
...
async fn socks5_connect(proxy_port: u16, target_port: u16) -> io::Result<TcpStream> {
    ...
    let mut method = [0u8; 2]; stream.read_exact(&mut method).await?;
    ...
    let mut reply = [0u8; 10]; stream.read_exact(&mut reply).await?;
...
async fn run_stress_case(proxy_port: u16, target_port: u16, concurrency: usize, payload: Arc<[u8]>) -> io::Result<Duration> {
    ...
    if pattern == "staged" {
        for batch_start in (0..concurrency).step_by(batch_size) {
            let batch_end = (batch_start + batch_size).min(concurrency);
    ...
    for (index, task) in tasks.into_iter().enumerate() {
        task.await.map_err(|e| io::Error::other(format!("stress task {index} join: {e}")))??;
    }
    Ok(start.elapsed())
}

fn child_stderr(child: &mut Child) -> String {
    child.stderr.take().map(|mut stderr| { use std::io::Read; let mut text = String::new(); let _ = stderr.read_to_string(&mut text); text }).unwrap_or_default()
}
...
async fn main() -> io::Result<()> {
    ...
    if result.is_err() {
        kill_child(&mut client); kill_child(&mut server);
        let client_log = child_stderr(&mut client); let server_log = child_stderr(&mut server);
    ...
    } else { kill_child(&mut client); kill_child(&mut server); }
    echo_task.abort();
    result
}

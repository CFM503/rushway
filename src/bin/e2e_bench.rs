...
fn child_path() -> io::Result<std::path::PathBuf> {
    ...
    let path = exe
        .parent()
        .map(|p| {
            p.join(if cfg!(windows) { "rushway.exe" } else { "rushway" })
        })
        .ok_or_else(|| {
            io::Error::new(io::ErrorKind::NotFound, "cannot locate sibling rushway binary")
        })?;
    if !path.exists() {
...
fn spawn_rushway(path: &std::path::Path, port: u16, upstream: Option<String>, key: &str, allow_local_targets: bool) -> io::Result<Child> {
...
async fn one_flow(proxy_port: u16, target_port: u16, payload: Arc<[u8]>) -> io::Result<usize> {
    timeout(FLOW_TIMEOUT, async {
        ...
        if echoed.as_slice() != payload.as_ref() { return Err(io::Error::new(io::ErrorKind::InvalidData, "echo payload mismatch")); }
        Ok(payload.len())
...
async fn run_case(proxy_port: u16, target_port: u16, concurrency: usize, payload: Arc<[u8]>) -> io::Result<f64> {
    ...
    for _ in 0..concurrency { tasks.push(tokio::spawn(one_flow(proxy_port, target_port, Arc::clone(&payload)))); }
    let mut total = 0usize;
    for task in tasks { total += task.await.map_err(|e| io::Error::other(e.to_string()))??; }
    let secs = start.elapsed().as_secs_f64();
    Ok((total as f64 / (1024.0 * 1024.0)) / secs)
}

...
fn configured_mux_sessions() -> Option<usize> {
    std::env::var("RUSHWAY_E2E_MUX_SESSIONS")
        .ok()
...
fn child_path() -> io::Result<std::path::PathBuf> {
    let path = std::env::current_exe()?.parent().map(|p| p.join(if cfg!(windows) { "rushway.exe" } else { "rushway" })).ok_or_else(|| io::Error::new(io::ErrorKind::NotFound, "cannot locate sibling rushway binary"))?;
    if !path.exists() { return Err(io::Error::new(io::ErrorKind::NotFound, format!("missing {}", path.display()))); }
    Ok(path)
}

fn transport_mode() -> &'static str {
    match std::env::var("RUSHWAY_E2E_TRANSPORT").ok().as_deref() { Some("quic") => "quic", Some("wss") => "wss", _ => "ws" }
}
fn stress_mode() -> bool { matches!(std::env::var("RUSHWAY_E2E_STRESS").ok().as_deref(), Some("1" | "true" | "yes")) }
fn stress_pattern() -> &'static str { match std::env::var("RUSHWAY_E2E_STRESS_PATTERN").ok().as_deref() { Some("staged") => "staged", _ => "burst" } }
fn upstream_url(port: u16) -> String { match transport_mode() { "quic" => format!("quic://127.0.0.1:{port}"), "wss" => format!("wss://127.0.0.1:{port}/"), _ => format!("ws://127.0.0.1:{port}/") } }
    ...
    let task = tokio::spawn(async move {
        loop {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    let n = match socket.read(&mut buf).await { Ok(0) | Err(_) => break, Ok(n) => n };
...
async fn socks5_stage<T>(stage: &'static str, future: T) -> io::Result<T::Output> where T: std::future::Future {
    timeout(Duration::from_secs(10), future).await.map_err(|_| io::Error::new(io::ErrorKind::TimedOut, format!("SOCKS5 stage {stage} timed out after 10s")))
}

async fn socks5_connect(proxy_port: u16, target_port: u16) -> io::Result<TcpStream> {
    let mut stream = socks5_stage("tcp_connect", TcpStream::connect(("127.0.0.1", proxy_port))).await??;
    socks5_stage("greeting_write", stream.write_all(&[5, 1, 0])).await??;
    let mut method = [0u8; 2];
    socks5_stage("method_read", stream.read_exact(&mut method)).await??;
    if method != [5, 0] { return Err(io::Error::new(io::ErrorKind::PermissionDenied, "SOCKS5 no-auth was rejected")); }
    let port = target_port.to_be_bytes();
    socks5_stage("connect_write", stream.write_all(&[5, 1, 0, 1, 127, 0, 0, 1, port[0], port[1]])).await??;
    let mut reply = [0u8; 10];
    socks5_stage("connect_reply_read", stream.read_exact(&mut reply)).await??;
    if reply[1] != 0 { return Err(io::Error::new(io::ErrorKind::ConnectionRefused, format!("SOCKS5 connect failed: {}", reply[1]))); }
    Ok(stream)
}

async fn one_flow(index: usize, proxy_port: u16, target_port: u16, payload: Arc<[u8]>) -> io::Result<usize> {
    let result = timeout(FLOW_TIMEOUT, async {
        let mut stream = socks5_connect(proxy_port, target_port).await.map_err(|e| io::Error::other(format!("flow {index} connect: {e}")))?;
        stream.write_all(&payload).await.map_err(|e| io::Error::other(format!("flow {index} write: {e}")))?;
        let mut echoed = vec![0u8; payload.len()];
        stream.read_exact(&mut echoed).await.map_err(|e| io::Error::other(format!("flow {index} read echo: {e}")))?;
        if echoed.as_slice() != payload.as_ref() { return Err(io::Error::other(format!("flow {index} echo payload mismatch"))); }
        Ok(payload.len())
    }).await;
    match result { Ok(value) => value, Err(_) => Err(io::Error::new(io::ErrorKind::TimedOut, format!("flow {index} timed out after 30s"))) }
}

async fn run_case(proxy_port: u16, target_port: u16, concurrency: usize, payload: Arc<[u8]>) -> io::Result<f64> {
    let start = Instant::now();
    let tasks = (0..concurrency).map(|i| tokio::spawn(one_flow(i, proxy_port, target_port, Arc::clone(&payload)))).collect::<Vec<_>>();
    let mut total = 0usize;
    for task in tasks { total += task.await.map_err(|e| io::Error::other(e.to_string()))??; }
    Ok((total as f64 / (1024.0 * 1024.0)) / start.elapsed().as_secs_f64())
}

async fn run_stress_case(proxy_port: u16, target_port: u16, concurrency: usize, payload: Arc<[u8]>) -> io::Result<Duration> {
    let start = Instant::now();
    ...
    if pattern == "staged" {
        for batch_start in (0..concurrency).step_by(100) {
            ...
            eprintln!("stress launch batch start={batch_start} end={batch_end} total={concurrency}");
            for index in batch_start..batch_end { tasks.push(tokio::spawn(one_flow(index, proxy_port, target_port, Arc::clone(&payload)))); }
            sleep(Duration::from_millis(50)).await;
        }
    } else {
        for index in 0..concurrency { tasks.push(tokio::spawn(one_flow(index, proxy_port, target_port, Arc::clone(&payload)))); }
    }
    for (index, task) in tasks.into_iter().enumerate() { task.await.map_err(|e| io::Error::other(format!("stress task {index} join: {e}")))??; }
    Ok(start.elapsed())
}

fn process_fd_count(pid: u32) -> Option<usize> {
    #[cfg(target_os = "linux")] { std::fs::read_dir(format!("/proc/{pid}/fd")).ok().map(|it| it.count()) }
    #[cfg(not(target_os = "linux"))] { let _ = pid; None }
}
fn child_stderr(child: &mut Child) -> String { child.stderr.take().map(|mut stderr| { use std::io::Read; let mut text = String::new(); let _ = stderr.read_to_string(&mut text); text }).unwrap_or_default() }
fn kill_child(child: &mut Child) { let _ = child.kill(); let _ = child.wait(); }
...
async fn main() -> io::Result<()> {
    ...
    let mut client = spawn_rushway(&rushway, client_port, Some(upstream_url(server_port)), TEST_KEY, true, false)?;
    ...
    if result.is_err() {
        eprintln!("stress trap: server_pid={} server_fd={:?} client_pid={} client_fd={:?}", server_pid, process_fd_count(server_pid), client_pid, process_fd_count(client_pid));
    ...
    echo_task.abort();
    if diagnostic_mode() { eprintln!("stress trap: diagnostic_mode=1; server_fd={:?} client_fd={:?}", process_fd_count(server_pid), process_fd_count(client_pid)); }
    result
}